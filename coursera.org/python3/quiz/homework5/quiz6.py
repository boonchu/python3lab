...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    while current_day < days_in_simulation:
        ...
        print current_day, time_to_bribe, savings, bribe_cost, total_salary, fx1(current_day), fx2(current_day)
...
if __name__ == '__main__':
    ...
    use the fact that each bribe happens once every 10 days to derive a polynomial expression
    for total salary earned in terms of the number of days d.
    
    As a check, this expression in d should evaluate exactly to the total salary earned by the
    end of the day of each bribe.

    Answer: (d/10*(d/10+1)*1000)/2
    '''
